...
trees_messy <- vancouver_trees %>%
  unite(col = species_messy, 
    common_name, species_name,
    sep = "/")
head(trees_messy)

trees_tidy <- trees_messy %>%
  separate(col = species_messy,
    into = c("common_name", "species_name"))
head(trees_tidy)
...
For research question 1, I wanted to visualize where trees were planted in each neighbourhood and what types of areas are they most commonly planted (i.e. side streets, main streets, highways, etc...). I would like to see the specific species or common name of tree planted in each neighbourhood.

For research question 2, it would be interesting to see what trees were planted in each neighbourhood to be able to ask further questions such as which species is most commonly planted in each area of Vancouver and is there a correlation between the average age of homes built in each area and time when trees were planted?
...
filter_red_maple <- vancouver_trees %>%
  filter(common_name == "RED MAPLE")
view(filter_red_maple)

## Drop N/A values in longitute and laditude
drop_na_lat_long <- filter_red_maple %>%
  drop_na("longitude", "latitude")
view(drop_na_lat_long)

## Arrange by date planted and dropping N/A values from date planted
arrange_tree_day <- drop_na_lat_long %>%
  drop_na("date_planted") %>%
  arrange("date_planted")
view(arrange_tree_day)

## Keep columns common_name, neighbourhood_name, height_range_id, diameter, date_planted, longitude, latitude.
drop_col_tree <- arrange_tree_day %>%
  select(c(common_name, neighbourhood_name, height_range_id, diameter, date_planted, longitude, latitude))
view(drop_col_tree)
clean_vancouver_trees <- drop_col_tree
...
**1**: I will reorder the bar graph based on descending order of number of trees in each neighbourhood using the fct_infreq function which orders by frequency. This will make it easier to see which neighbourhood has the most number of trees planted.

trees_bar_neighbourhood <- ggplot(vancouver_trees, aes(x = fct_infreq(neighbourhood_name))) +
    geom_bar() +
    ylab("Number of Trees") + xlab("Neighbourhood") +
    theme(axis.text.x = element_text(angle = 90))
trees_bar_neighbourhood
...
**3**: I separated the time to a new column and seperated it into year, so I can see the number of trees planted in each year.

split_tree_date <- clean_vancouver_trees %>%
  separate(col = date_planted, into = c("year", "month", "date"), sep = "-")
head(split_tree_date)


#mutate(date_planted = lubridate::ymd(YYYY),
#         year = year(date))
...
**Research Question**: Is there a relationship between tree diameter and the year the tree was planted?
...
write_csv(neighbourhood_trees, here::here("mini-project-2", "output", "neighbourhood_trees.csv"))
read_csv(here::here("mini-project-2", "output", "neighbourhood_trees.csv"))
...
saveRDS(lm_tree_diam_yr, here::here("mini-project-2", "output", "lm_tree_diam_yr"))
readRDS(here::here("mini-project-2", "output", "lm_tree_diam_yr"))
